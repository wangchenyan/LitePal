...
public class DeleteTest extends LitePalTestCase {
	...
	public void testDeleteById() {
		initJude();
		jude.save();
		int rowsAffected = LitePalCopy.delete(Student.class, jude.getId());
		assertEquals(1, rowsAffected);
		Student s = getStudent(jude.getId());
		assertNull(s);
	}
	...
	public void testDeleteWithNotExistsRecordById() {
		int rowsAffected = LitePalCopy.delete(Student.class, 998909);
		assertEquals(0, rowsAffected);
	}
	...
	public void testDeleteCascadeM2OAssociationsOnMSideById() {
		createClassroomStudentsTeachers();
		int rowsAffected = LitePalCopy.delete(Classroom.class, gameRoom.get_id());
		assertEquals(5, rowsAffected);
		assertNull(getClassroom(gameRoom.get_id()));
		assertNull(getStudent(jude.getId()));
		assertNull(getStudent(rose.getId()));
	...
	public void testDeleteAllCascadeM2OAssociationsOnMSide() {
		createClassroomStudentsTeachers();
		int rowsAffected = LitePalCopy.deleteAll(Classroom.class, "id = ?", gameRoom.get_id() + "");
		assertEquals(5, rowsAffected);
		assertNull(getClassroom(gameRoom.get_id()));
		assertNull(getStudent(jude.getId()));
		assertNull(getStudent(rose.getId()));
	...
	public void testDeleteCascadeM2OAssociationsOnOSideById() {
		...
		assertEquals(1, rowsAffected);
		assertNull(getStudent(jude.getId()));
		rowsAffected = LitePalCopy.delete(Student.class, rose.getId());
		assertEquals(1, rowsAffected);
		assertNull(getStudent(rose.getId()));
		rowsAffected = LitePalCopy.delete(Teacher.class, john.getId());
		assertEquals(1, rowsAffected);
		assertNull(getTeacher(john.getId()));
		rowsAffected = LitePalCopy.delete(Teacher.class, mike.getId());
		assertEquals(1, rowsAffected);
		assertNull(getTeacher(mike.getId()));
	}
	...
	public void testDeleteAllCascadeM2OAssociationsOnOSide() {
		createClassroomStudentsTeachers();
		int rowsAffected = LitePalCopy.deleteAll(Student.class, "id = ?", String.valueOf(jude.getId()));
		assertEquals(1, rowsAffected);
		assertNull(getStudent(jude.getId()));
		rowsAffected = LitePalCopy.deleteAll(Student.class, "id = ?", String.valueOf(rose.getId()));
		assertEquals(1, rowsAffected);
		assertNull(getStudent(rose.getId()));
		rowsAffected = LitePalCopy.deleteAll(Teacher.class, "id = ?", String.valueOf(john.getId()));
		assertEquals(1, rowsAffected);
		assertNull(getTeacher(john.getId()));
		rowsAffected = LitePalCopy.deleteAll(Teacher.class, "id = ?", String.valueOf(mike.getId()));
		assertEquals(1, rowsAffected);
		assertNull(getTeacher(mike.getId()));
	}
	...
	public void testDeleteCascadeO2OAssociationsById() {
		createStudentsTeachersWithIdCard();
		int affectedRows = LitePalCopy.delete(Student.class, jude.getId());
		assertEquals(2, affectedRows);
		assertNull(getStudent(jude.getId()));
		assertNull(getIdCard(judeCard.getId()));
		affectedRows = LitePalCopy.delete(IdCard.class, roseCard.getId());
		assertEquals(2, affectedRows);
		assertNull(getStudent(rose.getId()));
		assertNull(getIdCard(roseCard.getId()));
		affectedRows = LitePalCopy.delete(Teacher.class, john.getId());
		assertEquals(2, affectedRows);
		assertNull(getTeacher(john.getId()));
		assertNull(getIdCard(johnCard.getId()));
		affectedRows = LitePalCopy.delete(IdCard.class, mikeCard.getId());
		assertEquals(1, affectedRows);
		assertNull(getIdCard(mikeCard.getId()));
	}
	...
	public void testDeleteAllCascadeO2OAssociations() {
		createStudentsTeachersWithIdCard();
		int affectedRows = LitePalCopy.deleteAll(Student.class, "id = ?", String.valueOf(jude.getId()));
		assertEquals(2, affectedRows);
		assertNull(getStudent(jude.getId()));
		assertNull(getIdCard(judeCard.getId()));
		affectedRows = LitePalCopy.deleteAll(IdCard.class, "id = ?", roseCard.getId() + "");
		assertEquals(2, affectedRows);
		assertNull(getStudent(rose.getId()));
		assertNull(getIdCard(roseCard.getId()));
		affectedRows = LitePalCopy.deleteAll(Teacher.class, "id = ?", "" + john.getId());
		assertEquals(2, affectedRows);
		assertNull(getTeacher(john.getId()));
		assertNull(getIdCard(johnCard.getId()));
		affectedRows = LitePalCopy.deleteAll(IdCard.class, "id=?", "" + mikeCard.getId());
		assertEquals(1, affectedRows);
		assertNull(getIdCard(mikeCard.getId()));
	}
	...
	public void testDeleteCascadeM2MAssociationsById() {
		createStudentsTeachersWithAssociations();
		int rowsAffected = LitePalCopy.delete(Teacher.class, john.getId());
		assertEquals(2, rowsAffected);
		assertNull(getTeacher(john.getId()));
		assertM2MFalse(studentTable, teacherTable, rose.getId(), john.getId());
		assertM2M(studentTable, teacherTable, rose.getId(), mike.getId());
	...
	public void testDeleteAllCascadeM2MAssociations() {
		createStudentsTeachersWithAssociations();
		int rowsAffected = LitePalCopy.deleteAll(Teacher.class, "id=?", "" + john.getId());
		assertEquals(2, rowsAffected);
		assertNull(getTeacher(john.getId()));
		assertM2MFalse(studentTable, teacherTable, rose.getId(), john.getId());
		assertM2M(studentTable, teacherTable, rose.getId(), mike.getId());
		assertM2M(studentTable, teacherTable, jude.getId(), mike.getId());
		createStudentsTeachersWithAssociations();
		rowsAffected = LitePalCopy.deleteAll(Teacher.class, "id=?", "" + mike.getId());
		assertEquals(3, rowsAffected);
		assertNull(getTeacher(mike.getId()));
		assertM2MFalse(studentTable, teacherTable, rose.getId(), mike.getId());
		assertM2MFalse(studentTable, teacherTable, jude.getId(), mike.getId());
	...
	public void testDeleteAllCascadeWithConditions() {
		...
		s3.save();
		int rows = LitePalCopy.deleteAll(Classroom.class, "name = ?", classroom.getName());
		assertEquals(3, rows);
		...
		rows = LitePalCopy.deleteAll(Classroom.class, "name = ?", classroom2.getName());
		assertEquals(2, rows);
	...
	public void testDeleteAll() {
		...
		int affectedRows = LitePalCopy.deleteAll(Student.class, "name = ? and age = ?", "Dusting",
				"10088");
		assertEquals(1, affectedRows);
		assertNull(getStudent(ids[2]));
		affectedRows = LitePalCopy.deleteAll(Student.class, "name = ? and age > ? and age < ?", "Dusting", "10085", "10092");
    ...
    public void testMarkAsDeleted() {
        ...
        List<Student> list = LitePalCopy.where("name=?", "Dusting").find(Student.class);
        assertTrue(list.size() >= 5);
        LitePalCopy.deleteAll(Student.class, "name=?", "Dusting");
        list = LitePalCopy.where("name=?", "Dusting").find(Student.class);
	...
	public void testDeleteAllWithWrongConditions() {
		try {
            LitePalCopy.deleteAll(Student.class, "name = 'Dustin'", "aaa");
			fail();
		} catch (DataSupportException e) {
			assertEquals("The parameters in conditions are incorrect.", e.getMessage());
		}
		try {
            LitePalCopy.deleteAll(Student.class, null, null);
			fail();
		} catch (DataSupportException e) {
			assertEquals("The parameters in conditions are incorrect.", e.getMessage());
		}
		try {
            LitePalCopy.deleteAll(Student.class, "address = ?", "HK");
			fail();
    ...
    public void testDeleteWithGenericData() {
        ...
        Cursor c = LitePalCopy.findBySQL("select * from " + tableName + " where " + column + " = ?", String.valueOf(id));
        assertEquals(3, c.getCount());
        c.close();
        classroom.delete();
        c = LitePalCopy.findBySQL("select * from " + tableName + " where " + column + " = ?", String.valueOf(id));
        assertEquals(0, c.getCount());
        c.close();
        assertFalse(classroom.isSaved());
        classroom.save();
        assertTrue(classroom.isSaved());
        c = LitePalCopy.findBySQL("select * from " + tableName + " where " + column + " = ?", String.valueOf(classroom.get_id()));
        assertEquals(3, c.getCount());
        c.close();
        LitePalCopy.deleteAll(Classroom.class, "id = ?", String.valueOf(classroom.get_id()));
        c = LitePalCopy.findBySQL("select * from " + tableName + " where " + column + " = ?", String.valueOf(classroom.get_id()));
        assertEquals(0, c.getCount());
        c.close();
    }
